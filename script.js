// script_modified.js – updated to integrate Firebase and grey out selected locations
// Firebase initialization
const firebaseConfig = {"apiKey": "AIzaSyC5MdUJBXSVnYW1k2e5kDBr7PUbDXTsNz0", "authDomain": "pv-delivery-project.firebaseapp.com", "databaseURL": "https://pv-delivery-project-default-rtdb.firebaseio.com", "projectId": "pv-delivery-project", "storageBucket": "pv-delivery-project.firebasestorage.app", "messagingSenderId": "887550029748", "appId": "1:887550029748:web:3caa2a9df6f26281c24ce1", "measurementId": "G-ZH9T5FFY80"};
...
function createNumberIcon(index, selected) {
    ...
    return L.divIcon({
        ...
        html: "<div style="background: " + background + "; color: white; border-radius: 12px; width: 24px; height: 24px; text-align: center; line-height: 24px;">" + (index + 1) + "</div>",
        iconSize: [24, 24],
        iconAnchor: [12, 24]
    });
}
...
points.forEach(function(pt, index) {
    var numberedIcon = createNumberIcon(index, false);
    var marker = L.marker([pt.lat, pt.lng], {icon: numberedIcon}).addTo(map);
...
// Leaderboard handling via Firebase Realtime Database
function updateLeaderboardDisplay() {
    var list = document.getElementById("leaderboardList");
    list.innerHTML = "";
    database.ref("leaderboard").orderByChild("score").limitToFirst(10).once("value").then(function(snapshot) {
...
function saveToLeaderboard(name, phone, score) {
    ...
    newEntry.set({ name: name, phone: phone, score: score });
    updateLeaderboardDisplay();
}